...
var search = function (query, callback, requestOptions) {
  ...
  requestOptions.url = 'https://www.ultimate-guitar.com/search.php?' + utils.encodeParams(query);
...
var autocomplete = function (query, callback, requestOptions) {
  ...
  requestOptions.url = 'https://www.ultimate-guitar.com/contribution/submit/autocomplete?' + utils.encodeParams(query);
  request(requestOptions, function(error, response, body) {
    if (error) {
      callback(error);
    } else if (response.statusCode != 200) {
